...
Even.changeLocationHref = function(link) {
  const locationLink = link || $('#bg-location');
  const isMobile = window.innerWidth <= 800;

  locationLink.attr(
    'href',
    isMobile ? 'https://goo.gl/maps/NqDbjXCuxdxpwEqu6' : 'https://goo.gl/maps/37TipuY9FfXt1wHX9'
  );

  $(window).resize(() => this.changeLocationHref(locationLink));
}

// long story
Even.pluralizeRussian = function() {
  const wordCount = $('.js-word-count');
  const readingTime = $('.js-reading-time');
  const words = ['слово', 'слова', 'слов'];
  const minutes = ['минута\xa0чтения', 'минуты\xa0чтения', 'минут\xa0чтения'];

  const pluralize = ($selectors, dictionary) => {
    $selectors.each((i, item) => {
      if (!item.innerText) {
        return;
      }

      const count = +item.innerText.trim().substr(-1);
      let plural = '';
      ...
      item.innerText = `${item.innerText}\xa0${plural}`;
    });
  };

  pluralize(wordCount, words);
  pluralize(readingTime, minutes);
}

// transforms YYYY-MM-DD to DD.MM.YYYY as russians normally use it
// Even.formatPostDate = function() {
//   const $dates = $('.js-post-date');
